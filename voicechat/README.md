...
Set `WEB_SEARCH_ENABLED=true` in `.env`. In `auto` mode, current or lookup-style questions are searched before the model answers. Search results are added to the model context with numbered citations and shown as clickable sources in the conversation. Use `WEB_SEARCH_MODE=always` to search every request or `WEB_SEARCH_MODE=off` to disable it.

The default `WEB_SEARCH_PROVIDER=duckduckgo` needs no key, but some networks block or throttle it. The project can also call the local BowAPI service: set `WEB_SEARCH_PROVIDER=bowapi` and `BOWAPI_SEARCH_URL=http://127.0.0.1:8090/answer`. Search failures fall back to the normal model response.
